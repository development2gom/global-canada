	  ...
	  <div class="row padding-vertical-30 padding-horizontal-50">
		...
		<?php foreach($photos as $photo){?>
		    <div class="dgom-ui-col-md-4 margin-bottom-20">
		      ...
		      <div class="dgom-ui-col-overlay dgom-ui-col-overlay-bg" style="background-image: url(<?php echo Yii::app ()->params ['pathBaseImages'].'con_'.$this->tokenContest."/idu_".$photo->txt_usuario_number."/small_".$photo->txt_file_name?>);">
		        ...
		        <!-- <img class="overlay-figure" src="<?php #echo Yii::app ()->params ['pathBaseImages']."idu_".$photo->txt_usuario_number."/small_".$photo->txt_file_name?>" alt="<?php # echo $photo->txt_pic_name?>"> -->
		          ...
		          <?php if(number_format($photo->num_calificacion_nueva,0)>0){?>
		          <div class="progreso">
		            <div class="pie-progress pie-progress-sm" data-plugin="pieProgress" data-barcolor="#75E268" data-size="100" data-barsize="4" data-goal="<?=number_format($photo->num_calificacion_nueva,0) ?>" aria-valuenow="<?=number_format($photo->num_calificacion_nueva,0) ?>" role="progressbar">
		              <div class="pie-progress-number"><?=number_format($photo->num_calificacion_nueva,0) ?></div>
		            </div>
		          </div>
		          <?php }?>
		        </figure>
