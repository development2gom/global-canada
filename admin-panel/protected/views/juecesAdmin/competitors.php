		...
		<div class="col-md-6">
			...
			<div class="dgom-ui-competitors-search">
				...
				echo CHtml::beginForm ( CHtml::normalizeUrl ( array (
						'adminPanel/competitors/'.$this->tokenContest 
				) ), 'get', array (
						'id' => 'filter-form' 
				) );
				?>
		...
		<div class="col-md-6 dgom-ui-competitors-search-radio">
			...
			echo CHtml::beginForm ( "", 'get', array (
				...
				CHtml::submitButton ( 'Exportar', array (
					"id" => "descargarArchivo",
					"encode" => false,
					"class"=>"btn btn-cv ladda-button dgom-ui-
-search-download",
					"data-style"=>"zoom-in"
				) )*/
				. CHtml::endForm ();
						...
						<div class="dgom-ui-drop-order">
							<div class="button-group">
								...
								<ul id="dropdown-menu">
									...
									<li><?=CHtml::link("Name", array('adminPanel/competitors/'.$this->tokenContest.'?string='.((isset ( $_GET ['string'] )) ? $_GET ['string'] : '').'&UsrUsuarios_sort_nombre='.((isset ( $_GET ['UsrUsuarios_sort'] )) ? $_GET ['UsrUsuarios_sort'] : 'txt_nombre')), array("class"=>"animsition-link"))?></li>
									<li><?=CHtml::link("Email", array('adminPanel/competitors/'.$this->tokenContest.'?string='.((isset ( $_GET ['string'] )) ? $_GET ['string'] : '').'&UsrUsuarios_sort_correo='.((isset ( $_GET ['UsrUsuarios_sort'] )) ? $_GET ['UsrUsuarios_sort'] : 'txt_correo')), array("class"=>"animsition-link"))?></li>
								</ul>
							</div>
						</div>
...
$(document).ready(function(){
	...
	function downloadFile(url, urlFile, id, l, competitors){
		$.ajax({
				url:url,
				data: competitors,
				type:"POST",
 ...
 ) . '&feedback="+feedback, "' .Configuracion::DOMINIO_SITIO. CHtml::normalizeUrl ( array (
  ...
  ) . '&feedback="+feedback+"&raw=RAW", "' . Configuracion::DOMINIO_SITIO.CHtml::normalizeUrl ( array (
		'juecesAdmin/downloadZIP' 
) ) . '", "zip", l, competitors);
		}
		// Clase active
