...
<div class="container">
	<div class="row">
		...
		// $command = Yii::app()->db->createCommand('CALL getPorcentajeAvance('.$cat->id_contest.', '.$cat->id_category.', @porcentaje)');
				...
				<div class="widget-content height-full">
					...
					<div class="grey-100 padding-10">
						...
						//if($cat->num_fotos_calificadas!=0){
							echo CHtml::link('<h2 class="dgom-ui-dashboard-box-heading dgom-ui-dashboard-box-heading-white">'.$cat->txt_name.'</h2>', array('adminPanel/photosCategory/'.$this->tokenContest.'/'.$cat->txt_token_category), array("class"=>"dgom-ui-link-dashboard-admin animsition-link"));
// 						}else{
...
// 						}
						...
						<div class="progress progress-lg dgom-ui-hr-progress-bar">
	                    	<div class="progress-bar progress-bar-primary progress-bar-striped" style="width: <?=round($cat->num_porcentaje_general)?>%;" role="progressbar"><?=round($cat->num_porcentaje_general)?>%</div>
	                    </div>
