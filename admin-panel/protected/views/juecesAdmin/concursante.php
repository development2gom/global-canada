...
$hasFeedBack = ViewUsersProducts::model ()->find ( array (
		"condition" => "txt_token='".$this->tokenContest."' AND id_usuario=:idUsuario",
		"params" => array (
...
$numPics = WrkPics::model ()->findAll ( array (
		"condition" => "b_status=2 AND ID=:idUsuario AND id_contest='".$concurso->id_contest."'",
		"params" => array (
				":idUsuario" => $data->id_usuario 
		) 
) );
...
$numPicsTotal = WrkPics::model ()->findAll ( array (
		"condition" => "ID=:idUsuario AND id_contest='".$concurso->id_contest."'",
		"params" => array (
				":idUsuario" => $data->id_usuario
		)
) );
		...
		<div class="dgom-ui-competitors-wrap-line-foto">
			<div class="dgom-ui-competitors-wrap-line-foto-int">
				...
				<?php }else if($data->b_login_social_network){?>
					<img src="http://graph.facebook.com/<?=$data->id_usuario_facebook?>/picture" alt="">
				<?php }else{?>
	...
	<div class="col-md-5">
		<div class="popup-gallery-<?=$data->txt_usuario_number?> dgom-ui-competitors-wrap-line-tolink dgom-ui-competitors-popup-gallery">
				...
				foreach ( $numPics as $pic ) {
					...
					?>
					<a href="<?= Yii::app()->params["pathBaseImages"].'con_'.$this->tokenContest.'/idu_' . $data->txt_usuario_number . "/large_" . $pic->txt_file_name?>" class="dgom-ui-competitors-wrap-line-numfotos-foto-box <?=$mencionFoto?>" style='background-image: url(<?php echo Yii::app()->params["pathBaseImages"].'con_'.$this->tokenContest.'/idu_' . $data->txt_usuario_number . "/small_" . $pic->txt_file_name?>);'>
...
$(document).ready(function(){
	...
	var url = "'.Yii::app()->createUrl("adminPanel/viewPhotosCompetitor/".$this->tokenContest).'";
	...
	$(".row-line-item-'.$data->txt_usuario_number.'").on("click", function(e){
		...
		console.log(e.target.className);
 		...
 		if( e.target.className=="dgom-ui-competitors-wrap-line-numfotos-foto-box " || e.target.className=="dgom-ui-tolink" || e.target.className=="dgom-ui-competitors-wrap-line-numfotos-foto-box-r" || e.target.className=="dgom-ui-competitors-wrap-line-numfotos-foto-box" || e.target.className=="dgom-ui-competitors-wrap-line-numfotos-foto-box dgom-ui-competitors-wrap-line-numfotos-foto-box-r"  || e.target.nodeName=="INPUT"){
		//if (e.target !== this){
