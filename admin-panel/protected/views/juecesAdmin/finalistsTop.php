...
$li = '';
for($i = 1; $i <= 10; $i ++) {
	$li.= "<li>".CHtml::link($i, array("adminPanel/finalists/".$this->tokenContest."?places=".$i), array("class"=>"animsition-link"))."</li>"; 
...
foreach ( $categorias as $categoria ) {
	...
	$categoriaFinalizada = Yii::app()->db->createCommand()
		...
		$lugares = ViewCalificacionFinal::model ()->findAll ( array (
			...
			"order" => "num_calificacion_top DESC, num_calificacion DESC, num_calificacion_desempate DESC",
			'limit'=>20
	) );
				...
				<div
					...
					<figure class="overlay overlay-hover dgom-ui-overlay-cont">
						<img class="overlay-figure"
							src="<?php echo Yii::app ()->params ['pathBaseImages'].'con_'.$this->tokenContest."/idu_".$lugar->txt_usuario_number."/small_".$lugar->txt_file_name?>"
							alt="...">
...
<!-- 							<a 
					...
					<div class="progreso">
							<div class="pie-progress pie-progress-sm"
								...
								<div class="pie-progress-number"><?=round($lugar->num_calificacion)?></div>
							</div>
					</div>
					<?php 
						}
